...
app.post("/debug-test", (req, res) => {
  console.error(
    `[Server] /debug-test endpoint hit - path: ${req.path}, originalUrl: ${
      req.originalUrl
    }, method: ${req.method}, body: ${JSON.stringify(req.body)}`
  );
  res.json({ message: "Debug test endpoint is working!" });
});

// Authentication endpoint (must be before auth middleware)
app.post(API_ROUTES.AUTH_LOGIN, (req, res) => {
  console.error(
    `[Server] Login request received - path: ${req.path}, originalUrl: ${req.originalUrl}, method: ${req.method}`
  );
...
app.delete(
  API_ROUTES.SESSION_BY_ID,
  handleDeleteSession(sessions, deviceConnections, webUIConnections)
);
app.get(
  API_ROUTES.HEALTH,
  handleHealthCheck(sessions, deviceConnections, webUIConnections)
);

// 404 handler for debugging
app.use((req, res, next) => {
  console.error(
    `[Server] 404 - Route not found: ${req.method} ${
...
const gracefulShutdown = (signal) => {
  ...
  wss.clients.forEach((client) => {
    if (client.readyState === WS_CONFIG.READY_STATE.OPEN) {
      client.close(
        WS_CONFIG.CLOSE_CODES.NORMAL_CLOSURE,
  ...
  server.close(() => {
    ...
    console.error(
      "[Server] Shutdown complete. Server must be restarted to use again."
    );
    process.exit(0);
  });
...
server.listen(PORT, HOST, () => {
  ...
  console.error(
    `[Server] WebSocket endpoint: ${protocol}://${
